...
-- FLOOR + CONVERT + GETDATE
SELECT 
  FLOOR(CONVERT(FLOAT, (GETDATE() - CreationDate)) / 365.25) AS Age,
  *
FROM Host

-- Concatenation
SELECT
  Name + ' ' + Url AS NameAndUrl, 
  *
FROM Host
...
SELECT *
FROM Host AS H
INNER JOIN CheckInterval AS Ch
ON H.CheckIntervalId = Ch.CheckIntervalId

-- NULL in the right table
SELECT Ch.CheckIntervalId, Ch.FrequencyOfVerification, Ch.MonitoringMode, Ch.TemplateId, H.Domain, H.HostId
FROM CheckInterval AS Ch
LEFT OUTER JOIN Host AS H
ON Ch.CheckIntervalId = H.CheckIntervalId
WHERE 

-- Get all left record, where right check IS NULL
SELECT Ch.CheckIntervalId, Ch.FrequencyOfVerification, Ch.MonitoringMode, Ch.TemplateId, H.Domain, H.HostId
FROM CheckInterval AS Ch
LEFT OUTER JOIN Host AS H
ON Ch.CheckIntervalId = H.CheckIntervalId
